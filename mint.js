...
async function claimTokens() {
  ...
  try {
    ...
    const senderAccount = web3.eth.accounts.privateKeyToAccount(account);

    // Create a transaction object
    const transactionObject = {
      from: senderAccount.address,
      to: contractAddress,
      gasLimit: 300000, // Increase this value
      gasPrice: await web3.eth.getGasPrice(),
//      gasPrice: web3.utils.toHex(web3.utils.toWei('15', 'gwei')),
      data: contract.methods.claimTokens().encodeABI(),
      chainId: 61,
    };
    ...
    // Check ETH balance of the sender's account
    const ethBalance = await web3.eth.getBalance(senderAccount.address);

    console.log(`ETH Balance: ${web3.utils.fromWei(ethBalance, 'ether')} ETH`);

    // Check the current token balance
    const currentBalance = await contract.methods.balanceOf(toAddress).call();

    console.log(`Current Token Balance: ${currentBalance}`);

    // Stop the loop if the target balance is reached
    //if (currentBalance >= targetTokenBalance) {
    //  console.log(`Target Token Balance of ${targetTokenBalance} reached. Stopping the loop.`);
   //   return;
    //}
  } catch (error) {
  ...
  // Add a delay before the next iteration (adjust the time based on your requirements)
  setTimeout(claimTokens, 5000); // 5 seconds delay
}
